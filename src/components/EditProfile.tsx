...
export const EditProfile = ({user} : EditProfileInterfaceProps) => {
    ...
    const [firstName, setFirstName] = useState<string>(user?.data?.firstName);
      ...
      const [age, setAge] = useState<string>(user?.data?.age);
      const [gender, setGender] = useState<string>(user?.data?.gender);
      const [about, setAbout] = useState<string>(user?.data?.about);
      const [photoUrl, setPhotoUrl] = useState<string>(user?.data?.photoUrl);
    ...
    const handleSubmit = async () => {
        try {
           await axios.patch(`${BASE_URL}/profile/edit`, {firstName, lastName, age, gender, photoUrl, about}, {withCredentials: true}).then((user) => {
    ...
    return <div className="flex justify-center my-10 gap-2 m-5">
      ...
      <div className="card bg-base-200 w-96 shadow-sm">
        <div className="card-body">
          ...
          <div className="card-actions justify-center mt-2">
            <button className={`btn ${ success ? "btn-success" : "btn-primary" } w-full`} onClick={handleSubmit}>
              {`Submit${success ?  "ted ✅" : ""}`}
            </button>
          </div>
        </div>
